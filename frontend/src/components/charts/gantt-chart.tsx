          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <div className="grid grid-cols-12 gap-1 text-xs text-muted-foreground mb-2">
                {chartData.dates
                  .filter(
                    (_, index) => index % Math.max(1, Math.floor(chartData.totalDays / 12)) === 0
                  )
                  .slice(0, 12)
                  .map((date, index) => (
                          ...
                          <div className="bg-popover border rounded-lg shadow-lg p-3 min-w-64">
                            <div className="space-y-2">
                              ...
                              <div className="text-sm text-muted-foreground space-y-1">
                                ...
                                {task.metadata?.items && <div>Items: {task.metadata.items}</div>}
                              </div>
                            </div>
                          </div>
                        </div>
...
export const PharmaGanttPresets = {
  generateDeliveryTimeline: (orders: any[]) => {
    return orders.map((order, index) => ({
      ...
      status: order.status,
      progress: order.status === 'completed' ? 100 : order.status === 'in-progress' ? 60 : 0,
