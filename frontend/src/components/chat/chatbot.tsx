...
// Custom Logo Component - Replace with your PNG path
const SupplAILogo = ({ className = "h-8 w-8" }: { className?: string }) => (
  <img 
    src={logoImage}
    alt="SupplAI Logo"
    className={className}
        ...
        {isOpen ? (
          ...
          >
            <Card className={`w-96 shadow-2xl border-0 ${isMinimized ? 'h-16' : 'h-[500px]'} transition-all duration-300`}>
              <CardHeader className="flex flex-row items-center justify-between p-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-t-lg">
                <div className="flex items-center gap-2">
                  <SupplAILogo className="h-6 w-6" />
                  <CardTitle className="text-sm font-medium">SupplAI Assistant</CardTitle>
                  <Badge variant="secondary" className="text-xs bg-white/20 text-white border-0">
                    KAG Powered
                  </Badge>
                </div>
          ...
          >
            ...
            <AnimatePresence>
              {showPopup && (
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                  className="absolute bottom-full right-0 mb-3 bg-gray-900 text-white px-3 py-2 rounded-lg shadow-lg whitespace-nowrap text-sm"
                >
                  How can I help you? 😊
            ...
            <Button
              onClick={() => setIsOpen(true)}
              className="w-16 h-16 rounded-full bg-white hover:bg-gray-50 shadow-lg border-4 border-blue-200 hover:border-blue-300 transition-all duration-200"
            >
              <SupplAILogo className="h-10 w-10" />
            </Button>
          </motion.div>
        )}
      </AnimatePresence>
