...
populate an SQLite DB and export CSVs into ./static/.
...
MEDICATION_PROFILES = [
    ...
    {"name": "Ibuprofen 200mg", "category": "Intermittent", "base_daily_demand": (3, 15)},
    {"name": "Paracetamol 500mg", "category": "Intermittent", "base_daily_demand": (5, 20)},
    {"name": "Salbutamol Inhaler", "category": "Chronic", "base_daily_demand": (6, 25)},
    {"name": "Insulin Vial", "category": "Chronic", "base_daily_demand": (4, 18)},
    {"name": "Hydrocortisone 2%", "category": "Sporadic", "base_daily_demand": (0.5, 4)},
    {"name": "Amoxicillin 500mg", "category": "Sporadic", "base_daily_demand": (0.8, 6)},
...
def generate_price_history_for_med(med_id, start_date, days):
    ...
    annual_price_drift = np.random.normal(0.0, 0.015)  # 1.5% annual drift (inflation, etc.)
    ...
    while t < days:
        ...
        if t == 0 or np.random.random() < 0.15:  # 15% chance of price adjustment
            ...
            change_factor = 1.0 + np.random.normal(0.0, 0.008)  # 0.8% volatility instead of 3%
            base_price = round(max(0.01, base_price * change_factor * trend_adjustment), 2)
    ...
    for sku in skus_shuffled:
        ...
        for split_idx in range(locations_to_assign):
            ...
            if compatible_locs:
                ...
                if random.random() < 0.70 and any(loc["item_count"] > 0 for loc in compatible_locs):
                    # Prefer locations that already have items (creates clustering)
                    weighted_locs = [loc for loc in compatible_locs if loc["item_count"] > 0]
                    if weighted_locs:
                ...
                ambient_locs = [loc for loc in loc_caps if loc["zone_type"] == "ambient" 
                ...
                if ambient_locs:
                    ...
                    assignments.append(
                        {"med_id": sku["med_id"], "location_id": fallback_loc["location_id"]}
                    )
                else:
                    # Truly orphaned
                    assignments.append({"med_id": sku["med_id"], "location_id": None})
...
def validate_generated_data(conn, meds, output_dir):
    ...
    if chronic_meds:
        ...
        for med_id, avg_daily, zero_days, total_days in cur.fetchall():
            med_name = next((m["name"] for m in meds if m["med_id"] == med_id), f"Med {med_id}")
            zero_pct = (zero_days / total_days) * 100 if total_days > 0 else 0
            
            if avg_daily < 5:
                category_issues.append(f"Chronic medication '{med_name}' has unusually low consumption: {avg_daily:.1f} units/day")
            if zero_pct > 15:
                category_issues.append(f"Chronic medication '{med_name}' has {zero_pct:.1f}% zero-demand days (should be <15%)")
    ...
    for med_id, _changes, price_range, avg_price in cur.fetchall():
        ...
        if volatility_pct > 25:  # More than 25% price variation
            med_name = next((m["name"] for m in meds if m["med_id"] == med_id), f"Med {med_id}")
            price_issues.append(f"Medication '{med_name}' has high price volatility: {volatility_pct:.1f}%")
    ...
    if slot_distribution:
        ...
        if slot_distribution:
            ...
            empty_locations = len([loc for loc, count in slot_distribution if count == 0])
            
            validation_report["summary"]["max_items_per_location"] = max_items
            validation_report["summary"]["empty_locations"] = empty_locations
            validation_report["summary"]["location_utilization_pct"] = round(
                ((len(slot_distribution) - empty_locations) / len(slot_distribution)) * 100, 1
            ) if len(slot_distribution) > 0 else 0
    ...
    if stockout_rate > 15:  # >15% stockout rate is unrealistic
        validation_report["warnings"].append(f"High stockout rate: {stockout_rate:.1f}% (should be <15%)")
    ...
    fragmentation_rate = len(fragmented_skus) / total_meds * 100 if total_meds > 0 else 0
    
    # 7. Count orphaned items (unassigned)
    cur.execute("SELECT COUNT(*) FROM sku_meta WHERE med_id NOT IN (SELECT med_id FROM slot_assignment_simple)")
    ...
    validation_report["summary"] = {
        ...
        "price_changes_per_med": round(sum(count for _, count in cur.execute("SELECT med_id, COUNT(*) FROM drug_prices GROUP BY med_id").fetchall()) / total_meds, 1) if total_meds > 0 else 0,
...
def generate_all(
        ...
        for med in tqdm(meds, desc=f"meds (store {store['store_id']})", leave=False):
            ...
            if cat == "Chronic":
                if profile:
                    ...
                    base = float(np.random.uniform(profile["base_daily_demand"][0], profile["base_daily_demand"][1]))
                else:
    ...
    print("="*50)
    print(f"Total Records: {validation_report['summary']['total_consumption_records']:,}")
    ...
    print(f"SKU Fragmentation Rate: {validation_report['summary']['sku_fragmentation_rate_pct']}%")
    ...
    if 'max_items_per_location' in validation_report['summary']:
        print(f"Max Items Per Location: {validation_report['summary']['max_items_per_location']}")
        print(f"Location Utilization: {validation_report['summary']['location_utilization_pct']}%")
    ...
    print("="*50)
    print("Validation report saved to:", os.path.join(output_dir, "validation_report.json"))
...
def parse_args():
    ...
    p.add_argument("--out", default="static", help="Output CSV directory")
