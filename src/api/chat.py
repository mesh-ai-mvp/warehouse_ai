...
class KAGOpenAILangChainAgent:
    ...
    def find_database_file(self):
        ...
        logger.error("Database file 'poc_supplychain.db' not found in any expected location!")
            ...
            with sqlite3.connect(self.db_path) as conn:
                for table_name, table_info in schema.items():
                    try:
                        ...
                        if len(df) > 0:
                            logger.debug(f"Sample row from {table_name}: {df.iloc[0].to_dict()}")
    ...
    def log_initialization_status(self):
        ...
        for table_name, df in self.db_data.items():
            logger.info(f"  - {table_name}: {len(df)} rows, columns: {list(df.columns)}")
        ...
        logger.info(f"OpenAI client: {'Available' if self.openai_client else 'Not available'}")
        logger.info(f"LangChain client: {'Available' if self.langchain_llm else 'Not available'}")
        logger.info("==========================================")
    ...
    def setup_langchain_components(self):
        ...
        knowledge_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a specialized supply chain management AI assistant with access to real-time database data.
...
Remember: You have access to real supply chain database data. Use it to provide specific, actionable insights."""),
        ...
        analytics_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a supply chain data analyst with access to comprehensive database information.
...
{context}"""),
        ...
        general_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a supply chain management AI assistant.
...
- If you have data, use specific examples from it"""),
    ...
    def extract_searchable_fields(self, row):
        ...
        for col, val in row.items():
            if pd.notna(val):
                ...
                if any(keyword in col_lower for keyword in ['name', 'product', 'item', 'material', 'part']):
                    searchable['item_name'] = str(val)
                elif any(keyword in col_lower for keyword in ['stock', 'quantity', 'inventory', 'current']):
                    searchable['stock_level'] = val
                elif any(keyword in col_lower for keyword in ['supplier', 'vendor', 'manufacturer']):
                    searchable['supplier'] = str(val)
                elif any(keyword in col_lower for keyword in ['price', 'cost', 'value']):
                    searchable['price'] = val
                elif any(keyword in col_lower for keyword in ['status', 'alert', 'condition']):
    ...
    def create_enhanced_text_representation(self, table_name, row):
        ...
        for col, val in row.items():
            if pd.notna(val) and str(val).strip():
                ...
                if any(keyword in col_clean for keyword in ['stock', 'quantity', 'inventory', 'current']):
                    try:
                        val_num = float(val)
                        if val_num < 5:
                            text_parts.extend([
                ...
                if any(keyword in col_clean for keyword in ['product', 'item', 'material', 'part']):
                    text_parts.extend([
                        "supply chain item", 
                        "inventory product", 
                        "stock item",
    ...
    def detect_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        intent_patterns = {
            ...
            'pricing': [
                r'\b(price|cost|expensive|cheap|pricing|budget)\b'
            ],
            'supplier': [
                r'\b(supplier|vendor|manufacturer|distributor)\b'
            ],
            'orders': [
                r'\b(order|purchase|procurement|delivery)\b'
            ]
        }
        ...
        if any(phrase in query_lower for phrase in ['low stock', 'running low', 'shortage']):
        ...
        is_knowledge_query = (
            len(detected_intents) > 0 or 
            any(keyword in query_lower for keyword in [
                'inventory', 'stock', 'product', 'item', 'supplier', 'low', 'critical',
                'show', 'find', 'list', 'how many', 'total', 'count', 'price', 'order'
            ])
        )
        ...
        chain_type = 'analytics' if 'analytics' in detected_intents else (
            'knowledge' if is_knowledge_query else 'general'
        )
    ...
    def retrieve_knowledge(self, query: str, top_k: int = 15) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Found {len(valid_indices)} matches above threshold {threshold}")
            ...
            for idx in top_valid_indices:
                try:
                    result = {
                        ...
                        'content': self.get_structured_content(self.knowledge_metadata[idx])
                    }
                    results.append(result)
                    
                    logger.debug(f"Retrieved: {self.knowledge_metadata[idx]['table']} - similarity: {similarities[idx]:.3f}")
    ...
    def enhance_query(self, query: str) -> str:
        ...
        synonyms = {
            'low stock': [
                'low inventory', 'stock shortage', 'running low', 'critical stock', 
                'reorder level', 'depleted stock', 'insufficient inventory',
                'stock alert', 'inventory warning', 'shortage alert'
            ],
            'product': [
                'item', 'material', 'part', 'component', 'goods', 'merchandise',
                'supply', 'inventory item', 'stock item'
            ],
        ...
        if any(word in query_lower for word in ['how many', 'total', 'count']):
            enhanced_terms.extend(['inventory count', 'stock total', 'product quantity'])
            ...
            if rows:
                ...
                for col in sample_row.keys():
                    ...
                    if any(key in col_lower for key in [
                        'name', 'product', 'item', 'stock', 'quantity', 
                        'price', 'supplier', 'status', 'alert', 'category'
                    ]):
                ...
                for i, row_info in enumerate(rows[:10]):
                    ...
                    for col in all_columns[:6]:
                        if col in row_data:
                            ...
                            if pd.notna(value) and str(value).strip():
                                ...
                                if any(keyword in col.lower() for keyword in ['stock', 'quantity']):
                                    try:
                                        val_num = float(value)
                                        status = ""
                                        if val_num < 10:
                    ...
                    if important_data:
                        context_parts.append(f"Record {i+1}: {' | '.join(important_data)} (match: {similarity:.2f})")
                
                if len(rows) > 10:
                    context_parts.append(f"... and {len(rows) - 10} more matching records")
                
                total_records += len(rows)
            
            context_parts.append("")
        
        context_parts.append(f"**SUMMARY:** {len(tables_data)} table(s), {total_records} matching records")
    ...
    def convert_history_to_messages(self, conversation_history: List[Dict] = None) -> List:
    ...
    async def generate_langchain_response(self, query: str, knowledge_context: List[Dict[str, Any]], 
                                        intent: Dict[str, Any], conversation_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        if not self.langchain_llm or not self.langchain_chains:
            ...
            return await self.generate_openai_direct_response(query, knowledge_context, intent, conversation_history)
        ...
        try:
            ...
            chain = self.langchain_chains.get(chain_type, self.langchain_chains['knowledge'])
            
            logger.info(f"Using LangChain chain: {chain_type}")
            logger.debug(f"Context length: {len(context)} chars, History: {len(history)} messages")
            ...
            logger.info(f"LangChain response generated. Tokens: {cb.total_tokens}, Cost: ${cb.total_cost}")
            ...
            return {
                ...
                'sources_used': [item['metadata']['table'] for item in knowledge_context],
    ...
    async def generate_openai_direct_response(self, query: str, knowledge_context: List[Dict[str, Any]], 
                                            intent: Dict[str, Any], conversation_history: List[Dict] = None) -> Dict[str, Any]:
...
Please provide a detailed, specific response based on this database data."""
    ...
    def generate_fallback_response(self, knowledge_context: List[Dict[str, Any]]) -> str:
...
Please check your API configuration and try again."""
        ...
        for table_name, rows in tables_data.items():
            ...
            for row in rows:
                ...
                for key, value in row.items():
                    if pd.notna(value):
                        key_lower = key.lower()
                        if any(name_key in key_lower for name_key in ['name', 'product', 'item']):
                            item_name = str(value)
                        elif any(stock_key in key_lower for stock_key in ['stock', 'quantity', 'current']):
                            try:
                                current_stock = float(value)
                            except ValueError:
                                pass
                        elif any(supplier_key in key_lower for supplier_key in ['supplier', 'vendor']):
                            supplier = str(value)
                
                # Categorize by stock level
                if current_stock is not None:
                    if current_stock < 10:
                        critical_stock_items.append(f"CRITICAL: {item_name}: {current_stock} units (Supplier: {supplier})")
                    elif current_stock < 50:
                        low_stock_items.append(f"LOW: {item_name}: {current_stock} units (Supplier: {supplier})")
    ...
    async def process_chat(self, message: str, conversation_history: List[Dict] = None) -> ChatResponse:
        ...
        try:
            ...
            logger.info(f"Intent detected: {intent['primary_intent']} (confidence: {intent['confidence']:.2f})")
            ...
            if intent['use_langchain'] and self.langchain_llm:
                ...
                result = await self.generate_langchain_response(message, knowledge, intent, conversation_history)
            elif self.openai_client:
                debug_info['processing_method'] = 'openai_direct'
                logger.info("Using OpenAI direct processing")
                result = await self.generate_openai_direct_response(message, knowledge, intent, conversation_history)
            else:
            ...
            confidence = max(
                intent['confidence'], 
                0.8 if knowledge and len(knowledge) > 5 else 0.3
            )
            ...
            logger.info(f"Response generated successfully (method: {debug_info['processing_method']})")
            return response
...
async def get_system_status():
    ...
    try:
        ...
        test_knowledge = kag_openai_langchain_agent.retrieve_knowledge(test_query, top_k=5)
        
        return {
            'system_health': 'Healthy',
            'openai_direct_available': kag_openai_langchain_agent.openai_client is not None,
            ...
            'knowledge_base': {
                ...
                'total_records': sum(len(df) for df in kag_openai_langchain_agent.db_data.values()),
                'vectors_built': hasattr(kag_openai_langchain_agent, 'knowledge_vectors') and kag_openai_langchain_agent.knowledge_vectors is not None,
                'vector_dimensions': kag_openai_langchain_agent.knowledge_vectors.shape if kag_openai_langchain_agent.knowledge_vectors is not None else None,
                'test_retrieval_results': len(test_knowledge)
            },
            'langchain_chains': list(kag_openai_langchain_agent.langchain_chains.keys()) if kag_openai_langchain_agent.langchain_chains else [],
            'configuration': {
                'model': os.getenv('OPENAI_MODEL', 'gpt-3.5-turbo'),
                'temperature': float(os.getenv('OPENAI_TEMPERATURE', '0.3')),
                'max_tokens': int(os.getenv('OPENAI_MAX_TOKENS', '1500')),
                'similarity_threshold': float(os.getenv('KNOWLEDGE_SIMILARITY_THRESHOLD', '0.01'))
            }
        }
    except Exception as e:
