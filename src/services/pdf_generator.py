...
class PDFReportGenerator:
    ...
    def _create_cover_page(self, template: Dict[str, Any]) -> List:
        ...
        metadata = [
            ['Report Type:', template.get('type', 'General').title()],
            ['Generated:', datetime.now().strftime('%B %d, %Y at %I:%M %p')],
            ['Format:', 'PDF with AI Insights'],
            ['Frequency:', template.get('frequency', 'On-demand').title()]
        ]
    ...
    def _create_executive_summary(self, ai_insights: Dict[str, Any]) -> List:
        ...
        if ai_insights.get('confidence_score'):
            ...
            confidence_color = HexColor('#27ae60') if confidence > 0.7 else HexColor('#f39c12') if confidence > 0.4 else HexColor('#e74c3c')
    ...
    def _create_data_section(self, template: Dict[str, Any], data: List[Dict[str, Any]]) -> List:
        ...
        if not data:
            elements.append(Paragraph("No data available for this report.", self.styles['BodyText']))
            return elements

        # Create data table
        table_data = self._prepare_table_data(data[:50])  # Limit to first 50 rows for PDF
    ...
    def _create_ai_insights_section(self, ai_insights: Dict[str, Any]) -> List:
        ...
        elements.append(Paragraph("AI Insights & Analysis", self.styles['CustomHeading1']))
        elements.append(Spacer(1, 12))
        ...
        if ai_insights.get('patterns'):
            elements.append(Paragraph("Identified Patterns", self.styles['CustomHeading2']))
        ...
        if ai_insights.get('anomalies'):
            elements.append(Paragraph("Anomalies Detected", self.styles['CustomHeading2']))
        ...
        if ai_insights.get('predictions'):
            elements.append(Paragraph("Predictive Analysis", self.styles['CustomHeading2']))
        ...
        if ai_insights.get('risk_assessment'):
            ...
            risk_color = '#e74c3c' if risk_level in ['critical', 'high'] else '#f39c12' if risk_level == 'medium' else '#27ae60'
        ...
        if ai_insights.get('action_items'):
            ...
            action_data = [['Priority', 'Action', 'Owner', 'Timeline']]
            for item in ai_insights['action_items'][:8]:
                if isinstance(item, dict):
                    action_data.append([
                        item.get('priority', 'Medium').upper(),
                        item.get('action', 'Action required'),
                        item.get('owner', 'TBD'),
                        item.get('timeline', 'TBD')
                    ])

            if len(action_data) > 1:
                action_table = Table(action_data, colWidths=[1*inch, 3*inch, 1.5*inch, 1*inch])
    ...
    def _prepare_table_data(self, data: List[Dict[str, Any]]) -> List[List]:
        ...
        formatted_headers = [
            Paragraph(h.replace('_', ' ').title(), wrap_style) for h in headers
        ]
        ...
        for row in data:
            ...
            for header in headers:
                ...
                if isinstance(value, float):
                    # Format numbers
                    if 'price' in header.lower() or 'cost' in header.lower() or 'amount' in header.lower():
                        value = f"${value:,.2f}"
    ...
    def _calculate_column_widths(self, headers: List) -> List[float]:
        ...
        if num_cols <= 3:
            ...
            # For many columns, make them smaller
            col_width = available_width / min(num_cols, 8)
